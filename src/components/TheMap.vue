...
const captionMaxLabel = computed({
  get: () => {
    return showStationPopRatio.value
      ? d3.max(cantonsData, (d) =>
          round2decimals(d.stations[2024] / (d.population.years[2024] / 100000))
        )
      : d3.max(cantonsData, (d) => d.stations[2024])
  },
})
  ...
  if (!showStationPopRatio.value) {
    ...
    return d3.scaleSequential(
      [
        ...
        d3.max(
          cantonsData,
          (d) => d.stations[2024] / (d.population.years[2024] / 100000)
        ),
      ],
      d3.interpolateHslLong(accent_light, accent)
    )
...
const createMap = () => {
  ...
  const width =
    select('.section-map .wrapper').node().getBoundingClientRect().width * 0.8
        ...
        .on('mouseover', (event, d) => {
          ...
          tooltip
            .html(
              ...
              <p>Gare/100k habitants : ${round2decimals(
                canton.stations[2024] / (canton.population.years[2024] / 100000)
              )}</p>
            `
...
const createCaption = () => {
  ...
  select('.caption-svg').append('svg').attr('width', width).attr('height', 40)
...
const updateMapSize = () => {
  const width = select('.section-map .wrapper')
    .node()
    ...
    <div class="wrapper">
      ...
      <div class="map-info">
        ...
        <div class="input-wrapper">
          ...
          <span class="span-txt inactive"
            >Gares CFF pour 100 000 habitant.e.s</span
          >
        </div>
      </div>
    </div>
